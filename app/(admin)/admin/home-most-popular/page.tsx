  ...
  if (loading) {
    return (
      <div className={styles.row}>
        <Loader2 className={styles.spinner} size={24} /> Đang tải danh sách phổ biến...
      </div>
    );
  }
    ...
    <div className={styles.stack}>
      <div className={styles.card}>
        <div className={styles.row2}>
          ...
          <div>
            <h1 className={styles.title}>Most Popular Section</h1>
            <p className={styles.text}>
              Danh sách sắp xếp theo số lượng học viên giảm dần. Tích chọn để hiển thị ra
              trang chủ.
            </p>
          </div>
        </div>
                  ...
                  <tr key={course._id} className={styles.row4}>
                    ...
                    <td className={styles.headCell2}>
                      <button
                        ...
                        onClick={() =>
                          handleTogglePopular(course._id, !!course.isPopular)
                        }
                        className={`${styles.button4} ${
                          course.isPopular ? styles.button : styles.button2
                        } ${updatingId === course._id ? styles.button3 : ""}`}
                      >
                        <span
                          className={`${styles.label4} ${
                            course.isPopular ? styles.label2 : styles.label3
                          }`}
                        />
                      </button>
                    </td>
